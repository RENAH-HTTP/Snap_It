...
const ui = (function () {
  ...
  function renderLibrary() {
    ...
    for (let i = 0; i < totalSlots; i++) {
      ...
      const num = '<div class="pad-num">' + SEG(String(i + 1), 11) + '</div>';
  ...
  function renderMixer() {
    ...
    if (tracks.length === 0) {
      [46, 62, 78, 38].forEach(function (h, i) {
        ...
        slot.innerHTML = '<div class="mix-crayon" style="--v:' + h + '"></div>' +
          '<div class="mix-strip">' + SEG(String(i + 1), 11) + '</div>';
    ...
    tracks.forEach(function (track, idx) {
      ...
      strip.innerHTML = SEG(String(idx + 1), 11);
  ...
  function refreshTempoDisplays() {
    ...
    document.getElementById('bpm-value').innerHTML = SEG(String(bpm), 14);
    document.getElementById('bpm-value-big').innerHTML = SEG(String(bpm), 18);
    document.getElementById('swing-value').innerHTML =
      SEG((swing > 0 ? '+' : '') + swing, 16);
    document.getElementById('key-value').innerHTML =
      SEG(NOTES_SEG[((key % 12) + 12) % 12], 16);
  ...
  function setupTopBar() {
    ...
    // Segment digits for the TR tab numbers and the guide steps.
    document.querySelectorAll('.tab-button .num').forEach(function (el) {
      el.innerHTML = SEG(el.textContent.trim(), 16);
    });
    document.querySelectorAll('.guide-num[data-seg]').forEach(function (el) {
      el.innerHTML = SEG(el.getAttribute('data-seg'), 14);
    });
  ...
  function showToast(message) {
    ...
    if (!toast) {
      ...
      toast.style.cssText =
        ...
        'background:#F6F3EC;color:#17171B;border:1px solid #26262A;border-radius:10px;' +
        'box-shadow:0 10px 30px rgba(23,23,27,.25);' +
        'padding:11px 20px;font:700 12px Bahnschrift,"Segoe UI",system-ui,sans-serif;letter-spacing:.06em;' +
        'z-index:80;';
